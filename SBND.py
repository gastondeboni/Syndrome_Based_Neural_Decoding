...
time_init = time.time()
max_time = 23.95  # maximum simulation time in hours
...
along with other aworks associated with this application (see IEEExplore).
...
1) PCM_FORM is for the parity-check (PC) matrix structure:
    ...
    - 'sparsify1' applies a first order sparsifying algorithm
    - 'sparsify2' idem, 2nd order
...
Contact me (gdebonirovella@gmail.com) if you have questions or corrections.
'''

# %% CHOOSE SIMULATION SETTINGS IN THIS SECTION

# Modulator - example for 16-QAM: MOD_ORDER, MOD_TYPE = 4, 'QAM'
MOD_ORDER, MOD_TYPE = 1, 'PSK'  # 'QAM', 'PSK'

# Error correcting code
TYPE, n, k = 'BCH', 127, 64  # 'BCH' 'POLAR'
PCM_FORM = 'standright' # 'normal', 'standright', 'standleft', 'sparsify1', 'sparsify2', 'random'
bit_gen = 'ones'  # 'zeros', 'ones', 'random', '01'
INTERLEAVER = False  # True or False for the presence of a bit-interleaver

# Neural decoder
type_output = 'decoder' # select 'decoder' for message-oriented and 'denoiser' for codeword-oriented.
type_NN = 'RecECCT'  # 'MLP', 'RNN', 'ECCT', 'RecECCT', 'DoubleECCT'

# Training parameters
EbN0dB_train = [3, 6]  # min and max Eb/N0 used in training
batch_size = 2**2
epochs, patience = 2, 3000
time_lim_hours = 20
learning_rate = 5e-4

# Decoder testing parameters
EbN0dB_test = np.arange(0,6.5,1)
min_block_errors = 500
nb_sim_max = 100000
test_batch_size = 2**8

# Other parameters
VERBOSE, PROGRESS = 1, True # select "0, False" for no output, or "1, True" for all outputs

# %% Get modulator and code
const = F.get_modulator(MOD_TYPE, MOD_ORDER)
...
add_to_filename = f'{2**MOD_ORDER}{MOD_TYPE}-H{PCM_FORM}-gen_{bit_gen}' + '-interleaver'*INTERLEAVER
save_path = f'Simulations/{TYPE}-{n}-{k}/{n}-{k}_SBND-{type_NN}-{type_output}_{batch_size}bs_{EbN0dB_train[0]}-{EbN0dB_train[1]}dB_{add_to_filename}/'
...
if type_NN == 'RNN':
    RNN_steps, RNN_size_factor, depth = 5, 5, 4  # if it is RNN
...
decod_model.compile(optimizer=optimizer, loss=loss_fcn, metrics=[
                    [F.sign_error_logits], [F.fer_block_logits]])
...
# save model
decod_model.save(save_path+'decod_model')
...
for i_ebn0 in range(len(EbN0dB_test)):
    ...
    while (nb_block_errors < min_block_errors) & (i_sim < nb_sim_max) & (time.time()-time_init < max_time*3600):
        ...
        if not INTERLEAVER:
            ...
            c_inter = np.take_along_axis(codewords, idx_inter, axis=1)
